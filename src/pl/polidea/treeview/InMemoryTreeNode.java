...
class InMemoryTreeNode<T> implements Serializable {
    ...
    public int indexOf(final T idx) {
        return getChildIdList().indexOf(idx);
    }
    ...
    public synchronized InMemoryTreeNode<T> add(final int index, final T child, final boolean vis) {
        ...
        final InMemoryTreeNode<T> newNode = new InMemoryTreeNode<T>(child,
                getId(), getLevel() + 1, getId() == null ? true : vis);
